...
class EncryptionAudioApp:
    ...
    def encrypt_file(self):
        ...
        if len(key) != 8:
            messagebox.showerror("Error", "Key must be 8 characters long!")  # Menampilkan pesan error jika kunci tidak 8 karakter
            return
        ...
        try:
            ...
            des = DES.new(key.encode('utf-8'), DES.MODE_CBC)  # Membuat objek DES dengan mode CBC
            ...
            output_path = self.input_file_path + '_encrypt.bin'  # Menentukan jalur file output
    ...
    def decrypt_file(self):
        ...
        if not self.input_file_path or not self.input_file_path.endswith('_encrypt.bin'):
            messagebox.showerror("Error", "Please select an encrypted file (.bin) first!")  # Menampilkan pesan error jika tidak ada file terenkripsi yang dipilih
            return
        try:
            with open(self.input_file_path, 'rb') as file:
            ...
            des = DES.new(key.encode('utf-8'), DES.MODE_CBC, iv)  # Membuat objek DES dengan mode CBC dan IV
            ...
            # Menentukan nama file asli dan ekstensi
            original_extension = os.path.splitext(self.input_file_path)[0].split('_encrypt')[0]  # Menentukan ekstensi dengan benar
            output_path = original_extension  # Menentukan jalur file output
